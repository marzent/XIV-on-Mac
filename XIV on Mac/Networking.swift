...
extension HTTPClient {
    static func fetch(
        url: URL, headers: OrderedDictionary<String, String?>?,
        postBody: Data = Data()
    ) -> Response? {
    ...
    static func fetch(
        url: URL, headers: OrderedDictionary<String, String>? = nil,
        postBody: Data = Data()
    ) -> Response? {
    ...
    static func fetchFile(
        url: URL, destinationUrl: URL? = nil,
        headers: OrderedDictionary<String, String>? = nil, proxy: String? = nil,
        progressCallback: ((Int64, Int64, Int64) -> Void)? = nil
    ) throws {
        let destURL =
            destinationUrl
            ?? Util.cache.appendingPathComponent(url.lastPathComponent)
        ...
        guard
            let fileHandle = tempURL.withUnsafeFileSystemRepresentation({
                fopen($0, "wb")
            })
        else {
        ...
        try curl.set(
            option: CURLOPT_MAX_RECV_SPEED_LARGE,
            Int(HTTPClient.maxSpeed * 1_000_000))
        if let proxy = proxy {
        ...
        func writeFile(
            contents: UnsafeMutablePointer<Int8>?, size: Int, nmemb: Int,
        ...
        func updateProgress(
            ptr: UnsafeMutableRawPointer?, dltotal: Double, dlnow: Double,
            ultotal: Double, ulnow: Double
        ) -> Int32 {
            guard let progPtr = ptr else {
                return 0
            }
            let progress = Unmanaged<Progress>.fromOpaque(progPtr)
                .takeUnretainedValue()
        ...
        if let callback = progressCallback {
            ...
            try curl.set(
                option: CURLOPT_XFERINFODATA, Unmanaged.passUnretained(progress)
            )
            DispatchQueue.main.async {
                ...
                timer = Timer.scheduledTimer(
                    ...
                    let speed =
                        (progress.now - lastDownloadedBytes) / updateInterval
                    lastDownloadedBytes = progress.now
                    callback(
                        Int64(progress.total), Int64(progress.now), Int64(speed)
                    )
                }
            }
        }
        ...
        if response == 200 {
            ...
            Log.error(
                "Invalid response \(response) for file download \(url)!\n")
            try? Log.error(Data(contentsOf: tempURL))
