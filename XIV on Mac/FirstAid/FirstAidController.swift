...
class FirstAidController: NSViewController, NSTableViewDelegate, NSTableViewDataSource {
    @IBOutlet var cfgCheckTable: NSTableView?
    @IBOutlet var cfgCheckResultImage: NSImageView?
    @IBOutlet var cfgCheckResultLabel: NSTextField?

    var cfgProblems: [FFXIVCfgCheckCondition] = [] {
        didSet {
            guard let cfgCheckTable = cfgCheckTable else {
                return
            }
            cfgCheckTable.reloadData()
            guard let cfgCheckResultImage = cfgCheckResultImage, let cfgCheckResultLabel = cfgCheckResultLabel else {
                return
            }
