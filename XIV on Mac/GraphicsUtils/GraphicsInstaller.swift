...
struct GraphicsInstaller {
    ...
    private static let system32 = Wine.prefix.appendingPathComponent("drive_c/windows/system32")
        ...
        if !fm.contentsEqual(atPath: winDllPath, andPath: dll.path) {
            if fm.fileExists(atPath: winDllPath) {
                do {
                    ...
                    Log.error("[GraphicsInstaller] error renaming wine dx dll \(winDllPath)\n\(error)")
                }
            }
            do {
    ...
    static func restore(dllName: String) {
        ...
        if fm.fileExists(atPath: oldDllPath) {
            do {
                ...
                Log.information("[GraphicsInstaller] restored old wine dx dll \(oldDllPath) to \(winDllPath)")
            } catch {
                Log.error("[GraphicsInstaller] error restoring old wine dx dll \(oldDllPath)\n\(error)")
            }
        }
    }
