...
struct CharacterAppearanceData {
    public enum Genders: UInt8 {
        ...
        var localizedName: String.LocalizationValue {
            String.LocalizationValue(
                stringLiteral: String(format: "APPEARANCE_GENDER_%d", rawValue))
        }
    }
    ...
    public enum Races: UInt8 {
        ...
        var localizedName: String.LocalizationValue {
            String.LocalizationValue(
                stringLiteral: String(format: "APPEARANCE_RACE_%d", rawValue))
        }
    }
    ...
    public enum Tribes: UInt8 {
        ...
        var localizedName: String.LocalizationValue {
            String.LocalizationValue(
                stringLiteral: (String(format: "APPEARANCE_TRIBE_%d", rawValue))
            )
        }
...
struct CharacterDataSlot: Identifiable, Comparable {
    ...
    var displayName: String {
        return (name.count == 0)
            ? NSLocalizedString("BENCHMARK_CHARACTER_UNNAMED", comment: "")
            : name
    }
    ...
    private mutating func loadData() {
        ...
        modDate = try? path.resourceValues(forKeys: [
        ...
        if let fileHandle: FileHandle = try? FileHandle(forReadingFrom: path) {
            ...
            if let rawCharacterData: Data = try? fileHandle.read(
                upToCount: MemoryLayout<CharacterAppearanceDataRaw>.size)
            {
                // Get the attributes
                let byteArray: [UInt8] = rawCharacterData.map { $0 }
                let rawCharacter: CharacterAppearanceDataRaw =
                    byteArray.withUnsafeBufferPointer {
                        ($0.baseAddress!.withMemoryRebound(
                            to: CharacterAppearanceDataRaw.self, capacity: 1
                        ) { $0 })
                    }.pointee
                ...
                if let characterData: Data = try? fileHandle.read(
                    upToCount: 212)  // Files are only 212 bytes total.
                {
