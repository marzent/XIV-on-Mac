	...
	var body: some View 
	{
		List(selection: $selection)
		{
			ForEach (characterData) { oneCharacter in
				CharacterSlotView(characterData: oneCharacter, selected: (selection == oneCharacter.id))
			}
		}
		.listStyle(.bordered)
...
class CharacterListState: ObservableObject {
	...
	{
		targetCharacters = pad(cdsArray: Benchmark.findAvailableDemoCharacters().sorted(by: {$0.id < $1.id}),
							   toSize: 40)
	}
				...
				VStack{
					HStack{
						...
						VStack {
							...
							CharacterDataListView(characterData: $characterListState.sourceCharacters,
												  selection: $importableCharacterSelection)
							Button("BENCHMARK_SLOT_IMPORT_BUTTON")
							{
								if let importCharacter: CharacterDataSlot = characterListState.sourceCharacters.filter({ $0.id == importableCharacterSelection }).first
								{
									Benchmark.importCharacterData(character: importCharacter)
									characterListState.loadTargetFromDemo()
									importableCharacterSelection = nil
								}
							}
