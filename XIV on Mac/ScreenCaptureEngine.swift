...
class ScreenCaptureEngine: NSObject, @unchecked Sendable {
    ...
    private let videoSampleBufferQueue = DispatchQueue(
        label: "com.marzent.XIVOnMac.VideoCapture")
    private let audioSampleBufferQueue = DispatchQueue(
        label: "com.marzent.XIVOnMac.AudioCapture")
    ...
    func startCapture(
        ...
        do {
            ...
            Log.error(
                "XIV Screen Recording: Failed to start screen recording \(error)"
            )
        }
    }
    ...
    func stopCapture() async -> URL? {
        ...
        do {
            ...
            Log.error(
                "XIV Screen Recording: Error stopping screen recording \(error)"
            )
        }
        if let avWriter = self.avWriter {
        ...
        async
    {
        do {
            ...
            Log.error(
                "Failed to update the stream session: \(String(describing: error))"
            )
        }
    }
}
    ...
    SCStreamDelegate
{
    ...
    private func isValidFrame(for sampleBuffer: CMSampleBuffer) -> Bool {
        ...
        guard
            CVPixelBufferGetIOSurface(pixelBuffer)?.takeUnretainedValue()
                != nil,  // SurfaceRef/Backing IOSurface
            let contentRectDict = attachments[.contentRect],
            CGRect(dictionaryRepresentation: contentRectDict as! CFDictionary)
                != nil,  // contentRect
            attachments[.contentScale] as? CGFloat != nil,  // contentScale
            attachments[.scaleFactor] as? CGFloat != nil  // scaleFactor
        else {
            return false
        }
    ...
    func stream(_ stream: SCStream, didStopWithError error: Error) {
        Log.error(
            "XIV Screen Recording: An error occurred while capturing: \(error)")
    }
}
...
class AVWriter {
    ...
    private func getRecordingPath() -> URL {
        ...
        if let savedStringLocation = UserDefaults.standard.string(
            forKey: ScreenCaptureHelper.captureFolderPref)
        {
            recordingLocation = URL(filePath: savedStringLocation)
        } else {
            recordingLocation =
                FileManager.default.urls(
                    for: .moviesDirectory, in: .userDomainMask)[0]
        }
        ...
        Log.information(
            "Screen Recording: Creating new recording at \(finalPath)")
        return finalPath
    }
    ...
    func startRecording(height: Int, width: Int) {
        ...
        switch ScreenCaptureCodec(
            rawValue: UserDefaults.standard.integer(
                forKey: ScreenCaptureHelper.videoCodecPref))
        {
        case .hevc:
    ...
    func recordVideo(sampleBuffer: CMSampleBuffer) {
        ...
        if assetWriter.status == .unknown {
            ...
            Log.error(
                "Error writing video - \(assetWriter.error?.localizedDescription ?? "Unknown error")"
            )
        }
    }
